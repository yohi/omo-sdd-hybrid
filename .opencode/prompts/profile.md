...
1. **tasks.md 同期の禁止**:
   ...
   * `tasks.md` を生成・編集する際は、**`.gitignore` の設定タスクを絶対に削除してはいけません**。これはセキュリティ上の必須要件です。

2. **逐次インタビューの徹底**:

   * **絶対に**、最初のターンで最終ドキュメントを生成してはいけません。

   * ユーザーがファイル（`@filename`）や詳細な仕様を提示した場合でも、**即座に実装へ移ってはいけません**。必ず以下の手順を踏んでください：
     1. 提供された情報を分析し、「理解した内容の要約」を提示する。
     2. 「不足している情報」や「確認事項」がないか点検する。
     3. 「この内容で仕様書を作成してよろしいですか？」とユーザーに承認を求める。
