...
export async function validateGapInternal(state: State, options: ValidateGapOptions): Promise<string> {
  ...
  if (options.taskId && options.taskId !== state.activeTaskId) {
    const resolved = resolveTask(options.taskId);

    if (!resolved) {
      return `エラー: タスク ${options.taskId} が見つかりません (scope.md または tasks.md を確認してください)`;
    }

    if (resolved.task.scopes.length === 0) {
      return `エラー: タスク ${options.taskId} に Scope が定義されていません`;
    }
    effectiveAllowedScopes = resolved.task.scopes;
  }
