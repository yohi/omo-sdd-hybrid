...
- **コメント / ドキュメント / コミットメッセージ**: **日本語（JAPANESE）** を必須とします。
- **エラーメッセージ**: `E_CODE: 日本語メッセージ` の形式を遵守してください。
...
PR のレビュー・承認が完了した後、ユーザーがスラッシュコマンドを実行して仕様を確定させます。
...
2. **日本語仕様書のリネーム**: 各 `.md` ファイルを `*_ja.md` にリネームします（例: `requirements.md` → `requirements_ja.md`）。
3. **翻訳プロンプトの生成**: `*_ja.md` の内容を元に、英語の仕様書（Source of Truth）を作成するための翻訳指示がエージェントに提示されます。
...
| `profile` | インタビュー形式で要件を収集し、EARS記法の初期化プロファイルを日本語で生成します。環境が構造化質問ツールを提供していれば自動利用します。 |
...
**`.kiro/specs/**/scope.md` の Scope（glob）と、git差分で検出した変更ファイルを突合**して検証します。
Scope外の変更が含まれている場合は **Fail-Closed（CIを失敗）** します。

#### デフォルト挙動

- `specs/**`, `.opencode/**`, `.kiro/**` は Always Allow（Scope突合をスキップして許可）
- それ以外の変更は、`.kiro/specs/**/scope.md` に定義された Scope のいずれかに一致する必要があります
...
```bash
# Always Allow を無効化し、すべての変更が scope.md の Scope に含まれることを必須化
...
- `--strict`:
  - Always Allow（`specs/**`, `.opencode/**`, `.kiro/**`）を無効化し、これらのパスも Scope に含まれない場合は失敗します。
- `--allow-untracked`:
...
ホスト環境の汚染を防ぐため、ビルドやインストールなどのコマンドは **必ずコンテナ内** で実行してください。
ツール群は実行時にコンテナ環境かどうかをチェックし、ホスト環境の場合は `E_HOST_COMMAND_BLOCKED` エラーを発生させます。

### 主な開発コマンド

| コマンド | 説明 |
|---------|------|
| `bun run build` | プロジェクトを `dist/` にビルドします。 |
...
| `bun test <path>` | 単一のテストファイルを実行します（例: `bun test __tests__/tools/foo.test.ts`）。 |
| `bun run lint:md` | Markdownファイルを `markdownlint-cli2` でリントします。 |
